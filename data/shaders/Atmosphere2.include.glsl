...
const float ATMOSPHERE_HEIGHT = 100000;
...
const float ATMOSPHERE_DENSITY = 1;
const float EXPOSURE = 20;
...
vec3 IntegrateScattering(vec3 rayStart, vec3 rayDir, float rayLength, vec3 lightDir, vec3 lightColor, out vec3 transmittance)
{
    ...
    float sampleDistributionExponent = 1 + clamp(1 - rayHeight / ATMOSPHERE_HEIGHT, 0.0, 1.0) * 8; // Slightly arbitrary max exponent of 9
