...
namespace Rhi {
    ...
    struct TGraphicsPipeline : public TPipeline {
        ...
        auto BindBufferAsVertexBuffer(uint32_t buffer, uint32_t bindingIndex, size_t offset, size_t stride) const -> void;
        auto DrawArrays(int32_t vertexOffset, size_t vertexCount) const -> void;
        auto DrawElements(uint32_t indexBuffer, size_t elementCount) const -> void;
        auto DrawElementsInstanced(uint32_t indexBuffer, size_t elementCount, size_t instanceCount) const -> void;
...
namespace std {
    template<>
    struct hash<Rhi::TSamplerDescriptor> {
        size_t operator()(const Rhi::TSamplerDescriptor& samplerDescriptor) const {
            size_t seed = 0;
