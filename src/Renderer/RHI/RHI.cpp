...
auto FormatToUnderlyingOpenGLType(TFormat format) -> uint32_t {
    ...
    switch (format) {
        ...
        default:
            const std::string message = "Format not mappable to opengl type";
            glDebugMessageInsert(GL_DEBUG_SOURCE_APPLICATION, GL_DEBUG_TYPE_ERROR, 0, GL_DEBUG_SEVERITY_HIGH, message.size(), message.data());
            return -1;
    }
}
...
constexpr auto FormatToAttachmentType(TFormat attachmentFormat, size_t colorAttachmentIndex) -> TAttachmentType {
    ...
    switch (attachmentFormat) {
        ...
        default:
            return static_cast<TAttachmentType>(static_cast<std::underlying_type_t<TAttachmentType>>(TAttachmentType::ColorAttachment0) + colorAttachmentIndex);
    }
}
...
auto UploadTexture(const TTextureId& textureId, const TUploadTextureDescriptor& updateTextureDescriptor) -> void {
    ...
    const auto format = UploadFormatToGL(updateTextureDescriptor.UploadFormat == TUploadFormat::Auto
        ? FormatToUploadFormat(texture.Format)
        : updateTextureDescriptor.UploadFormat);

    const auto type = updateTextureDescriptor.UploadType == TUploadType::Auto
        ? FormatToUnderlyingOpenGLType(texture.Format)
        : UploadTypeToGL(updateTextureDescriptor.UploadType);
    ...
    for (auto colorAttachmentIndex = 0; auto colorAttachmentValue : framebuffer.ColorAttachments) {
        if (colorAttachmentValue.has_value()) {
            ...
            if (colorAttachment.LoadOperation == TFramebufferAttachmentLoadOperation::Clear) {
                switch (FormatToBaseTypeClass(colorAttachment.Texture.Format)) {
...
auto TGraphicsPipeline::BindBufferAsVertexBuffer(
    ...
    const uint32_t bindingIndex,
    const size_t offset,
    const size_t stride) const -> void {
...
auto TGraphicsPipeline::DrawArrays(
    ...
    const size_t vertexCount) const -> void {
...
auto TGraphicsPipeline::DrawElements(
    ...
    const size_t elementCount) const -> void {
...
auto TGraphicsPipeline::DrawElementsInstanced(
    ...
    const size_t instanceCount) const -> void {
